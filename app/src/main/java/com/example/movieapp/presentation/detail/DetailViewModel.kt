...
import com.example.movieapp.domain.use_case.get_videos_movie.GetVideosMovieUseCase
...
class DetailViewModel @Inject constructor(
    ...
    private val _videosState = mutableStateOf<DetailState>(DetailState())
    val videosState: State<DetailState> = _videosState
    ...
    private fun getVideosMovie(movieId: String) {
        getVideosMovieUseCase.executeGetVideosMovie(movieId = movieId) .onEach {
            when (it) {
                is Resource.Success -> {
                    _videosState.value = it.data?.let { it1 -> DetailState(video = it1) }!!
                }

                is Resource.Loading -> {
                    _videosState.value = DetailState(isLoading = true)
                }

                is Resource.Error -> {
                    _videosState.value = DetailState(error = it.message ?: "Error")
                }
            }
        }.launchIn(viewModelScope)
    }
    ...
    fun loadGetVideosMovie(movieId: String) {
